...
import type { IImageViewerImageMeta, ImageViewerMapMarker } from '../models/types'
...
const mapMarkers = computed((): ImageViewerMapMarker[] => {
  const coords = gpsCoordinates.value
  if (!coords)
    return []
  return [{
...
.metadata-panel {
  ...
  max-width: 420px;
  height: 100%;
  background: var(--bg-secondary-color, #1e1e1e);
  color: var(--fg-primary-color, #ffffff);
  display: flex;
  flex-direction: column;
  border-left: 1px solid var(--border-primary-color, rgba(255, 255, 255, 0.15));
  box-shadow: var(--s-l, 0 10px 30px rgba(0, 0, 0, 0.6));
  pointer-events: auto;
...
.close-btn-panel {
  ...
  border: none;
  cursor: pointer;
  color: var(--fg-tertiary-color, rgba(255, 255, 255, 0.5));
  font-size: 24px;
  display: flex;
  align-items: center;
...
.panel-content {
  ...
  &::-webkit-scrollbar-thumb {
    background: var(--border-primary-color, rgba(255, 255, 255, 0.2));
...
.extended-header {
  ...
  border-bottom: 1px solid var(--border-secondary-color, rgba(255, 255, 255, 0.1)) !important;
  ...
  button {
    ...
    transition: color 0.2s;
    &:hover {
      color: var(--bg-action-hover-color, #60a5fa);
    }
  }
}
...
.info-icon {
  font-size: 18px;
  color: var(--fg-muted-color, rgba(255, 255, 255, 0.4));
  flex-shrink: 0;
  width: 20px;
  text-align: center;
}
...
.extended-list {
  font-size: 0.85rem;
  background-color: var(--bg-tertiary-color, rgba(255, 255, 255, 0.05));
  border-radius: var(--r-s, 6px);
  padding: 0 12px;

  .info-item {
    border-color: var(--border-primary-color, rgba(255, 255, 255, 0.1));
  }
  dt {
