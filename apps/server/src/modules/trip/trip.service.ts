...
import { dayRepository } from '~/modules/day/day.repository'
...
export const tripService = {
  ...
  async create(data: z.infer<typeof CreateTripInputSchema>, userId: string) {
    ...
    try {
      // dayRepository тоже должен быть мигрирован, но здесь мы просто вызываем его метод
  ...
  async update(
    ...
    // Проверка прав. В репозитории id уже строка (UUID), userId тоже.
    // Важно: в базе userId это Record Link, но mapTrip превращает его в строку.
  ...
  async delete(id: string, userId: string, userRole: string) {
    ...
    // Если images реализованы
    const totalImageSize = (tripToDelete.images || []).reduce((sum: number, image: any) => sum + (image.sizeBytes || 0), 0)
    if (totalImageSize > 0)
