...
export interface IBlogRepository {
  ...
  delete: (id: string) => Promise<void>
  deleteImage: (params: { url: string }) => Promise<{ success: boolean }>
}
...
### 2. `shared / services / api / repositories / post.repository.ts`
Добавляем tRPC клиентский метод удаления медиа.
