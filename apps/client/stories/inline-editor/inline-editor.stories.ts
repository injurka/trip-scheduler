...
export const WithCallouts: Story = {
  render,
  args: {
    modelValue: `## Демонстрация стилизованных блоков (Callouts / Alerts)

> [!TIP]
> 1. Официальная лотерея «Taiwan the Lucky Land» — шанс выиграть ~14 000 ₽
> Зарегистрируйтесь на официальном сайте за 1–7 дней до прилета.

> [!NOTE]
> 🔌 Розетки, адаптеры и напряжение: в Тайване используется американский стандарт Type A / B, 110V.

> [!IMPORTANT]
> 📶 Удаленная работа и VPN: сервисы банков РФ требуют VPN с российскими IP-адресами.

> [!WARNING]
> 🚇 Правило «Желтой линии» в метро MRT: за желтой чертой запрещено есть, пить и жевать жвачку. Штраф до ~21 000 ₽.

> [!CAUTION]
> 🚫 СТРОЖАЙШИЙ ЗАПРЕТ: Любые мясные продукты категорически запрещены к ввозу на Тайвань (штраф ~560 000 ₽).

> Обычная стандартная цитата без типа алертов.
`,
  },
}
